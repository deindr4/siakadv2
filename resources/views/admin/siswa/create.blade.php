...
@section('page-subtitle', 'Input data siswa manual')
...
<div class="page-title">
    <h1>➕ Tambah Siswa</h1>
    <p>Input data siswa secara manual</p>
</div>
            ...
            <div class="card-body">
                <div style="display:flex;flex-direction:column;gap:14px;">
                        ...
                        <select name="rombongan_belajar_id" style="width:100%;padding:9px 14px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none;font-family:inherit;background:#fff;">
                            ...
                            @foreach($rombels as $r)
                                <option value="{{ $r->rombongan_belajar_id }}"
                                    ...
                                    {{ old('rombongan_belajar_id') == $r->rombongan_belajar_id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <input type="number" name="anak_keberapa" value="{{ old('anak_keberapa') }}" min="1"
                            style="width:100%;padding:9px 14px;border:1.5px solid #e2e8f0;border-radius:8px;font-size:13px;outline:none;font-family:inherit;">
                    </div>
...
<script>
// Auto-fill nama_rombel dan tingkat saat pilih rombel
document.querySelector('[name="rombongan_belajar_id"]').addEventListener('change', function() {
    const opt = this.options[this.selectedIndex];
    document.querySelector('[name="nama_rombel"]') && (document.querySelector('[name="nama_rombel"]').value = opt.dataset.nama || '');
    document.querySelector('[name="tingkat_pendidikan_id"]') && (document.querySelector('[name="tingkat_pendidikan_id"]').value = opt.dataset.tingkat || '');
});
</script>
@endsection
