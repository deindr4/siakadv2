{{-- resources/views/partials/sidebar_bk.blade.php --}}
{{-- Role: bk --}}
@php
    $ag = '';
    if (request()->is('dashboard/bk'))          $ag = 'utama';
    elseif (request()->is('bk/absensi*'))       $ag = 'absensi';
    elseif (request()->is('guru/jurnal*'))       $ag = 'guru';
    elseif (request()->is('bk/poin-positif*'))  $ag = 'poin';
...
<div class="nav-group">
    ...
    <div class="nav-group-body {{ $ag==='utama'?'open':'' }}">
        <div class="nav-item"><a href="{{ route('dashboard.bk') }}" class="{{ request()->is('dashboard/bk')?'active':'' }}"><i class="bi bi-grid-1x2-fill"></i> Dashboard</a></div>
    </div>
</div>

<div class="nav-group">
    <div class="nav-group-header {{ $ag==='absensi'?'open':'' }}" onclick="toggleGroup(this)">
        <i class="bi bi-calendar-check nav-gi"></i><span class="nav-gl">Absensi</span><i class="bi bi-chevron-down nav-gc"></i>
    </div>
    <div class="nav-group-body {{ $ag==='absensi'?'open':'' }}">
        <div class="nav-item"><a href="{{ route('bk.absensi.index') }}" class="{{ request()->is('bk/absensi') && !request()->is('bk/absensi/create*')?'active':'' }}"><i class="bi bi-list-ul"></i> Daftar Absensi</a></div>
        <div class="nav-item"><a href="{{ route('bk.absensi.create') }}" class="{{ request()->is('bk/absensi/create*')?'active':'' }}"><i class="bi bi-pencil-square"></i> Input Absensi</a></div>
    </div>
</div>
...
<div class="nav-group">
    <div class="nav-group-header {{ $ag==='poin'?'open':'' }}" onclick="toggleGroup(this)">
        <i class="bi bi-star-fill nav-gi" style="color:#fbbf24"></i><span class="nav-gl">Poin Kebaikan</span><i class="bi bi-chevron-down nav-gc"></i>
    </div>
    <div class="nav-group-body {{ $ag==='poin'?'open':'' }}">
        <div class="nav-item"><a href="{{ route('bk.poin-positif.index') }}" class="{{ request()->is('bk/poin-positif') || (request()->is('bk/poin-positif*') && !request()->is('bk/poin-positif/jenis*') && !request()->is('bk/poin-positif/rekap*')) ? 'active' : '' }}"><i class="bi bi-star-fill"></i> Input Poin Kebaikan</a></div>
...
{{-- IZIN BERENCANA — BK hanya lihat status --}}
<div class="nav-group">
    ...
    <div class="nav-group-body {{ $ag==='izin'?'open':'' }}">
        <div class="nav-item"><a href="{{ route('izin.index') }}" class="{{ request()->is('izin*')?'active':'' }}"><i class="bi bi-file-earmark-text-fill"></i> Daftar Izin</a></div>
    </div>
</div>
